...
public class ShadowCopyAction implements CopyAction {
    ...
    private final List<String> localExcludes
    ...
    public ShadowCopyAction(File zipFile, ZipCompressor compressor, DocumentationRegistry documentationRegistry,
                            ...
                            ShadowStats stats, List<String> localExcludes) {
        ...
        this.stats = stats
        this.localExcludes = localExcludes
    }
    ...
    class StreamAction implements CopyActionProcessingStreamAction {
        ...
        public StreamAction(ZipOutputStream zipOutStr, List<Transformer> transformers, List<Relocator> relocators,
                            PatternSet patternSet, ShadowStats stats, localExcludes) {
        ...
        private void visitFile(FileCopyDetails fileDetails) {
            if (!isArchive(fileDetails)) {
                try {
                    ...
                    boolean skip = false
                    String clazz = fileDetails.path.replace('/', '.')
                    for (String exclude : localExcludes) {
                        if (clazz.startsWith(exclude)) {
                            skip = true
                        }
                    }
                    if (!remapper.hasRelocators() || !isClass || skip) {
